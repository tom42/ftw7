...
glfw::monitor opengl_display_driver::find_monitor(const wchar_t* display_name)
{
    if (!display_name)
    {
        return m_glfw.get_primary_monitor();
    }

    for (auto monitor : m_glfw.get_monitors())
    {
...
glfw::window opengl_display_driver::create_window(HINSTANCE emulation_dll_module_handle, const ftw7_core::emulation::settings& settings)
{
    ...
    if (settings.fullscreen)
    {
        ...
        window = m_glfw.create_window(settings.screen_width, settings.screen_height, FTW7_OPENGL_DISPLAY_DRIVER_NAME, &monitor);
